...
void main(){
    ...
    if (bool(channelMask & CHANNEL_RED))   value.r = pow(value.r, exponent);
    if (bool(channelMask & CHANNEL_GREEN)) value.g = pow(value.g, exponent);
    if (bool(channelMask & CHANNEL_BLUE))  value.b = pow(value.b, exponent);
    if (bool(channelMask & CHANNEL_ALPHA)) value.a = pow(value.a, exponent);
